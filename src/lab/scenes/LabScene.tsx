...
export default function LabScene() {
  ...
  const [reactionState, setReactionState] = useState<{
    ...
    type: 'acid-base' | 'precipitation' | 'gas-evolution' | 'color-change' | 'heat-release' | 'boiling' | 'smoke' | 'crystallization';
    intensity: number;
    chemicals: string[];
    active: boolean;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    console.log('LabScene: available experiments:', ALL_EXPERIMENTS.map(exp => exp.id));
    if (slug) {
      const foundExperiment = ALL_EXPERIMENTS.find(exp => exp.id === slug);
